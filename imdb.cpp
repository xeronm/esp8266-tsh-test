...
protected:
	void SetUp()
	{
		...
		imdb_def_t db_def = { block_size-1, BLOCK_CRC_NONE };
		imdb_init(&db_def, &hmdb);
		imdb_class_def_t	cdef = { "testobj", false, false, false, 25, 3, 0, 8, obj_size - 1 };
...
protected:
	void SetUp()
	{
		...
		imdb_def_t db_def = { block_size - 1, BLOCK_CRC_NONE };
		imdb_init(&db_def, &hmdb);
		imdb_class_def_t	cdef = { "testobj", true, false, false, 25, 1, 0, 8, obj_size - 1 };
		imdb_class_create(hmdb, &cdef, &hcls);
	}
	void TearDown()
	{
...
protected:
	void SetUp()
	{
		...
		imdb_def_t db_def = { block_size, BLOCK_CRC_NONE };
		imdb_init(&db_def, &hmdb);
		{
...
TEST_F(IMDBFixedClass, PagesMaxFill)
{
	...
	ASSERT_EQ(imdb_inf.stat.slot_split, imdb_inf.stat.slot_data);
...
TEST_F(IMDBVariableClass, Insert128)
{
	...
	ASSERT_EQ(class_info.fl_skip_count, 70);
	ASSERT_EQ(class_info.slots_free_size, 6816);
